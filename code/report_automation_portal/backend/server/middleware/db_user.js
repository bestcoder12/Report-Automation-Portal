...
const userOps = async (db) => {
    const chkPassStrngth = (ptPassword, strngthLvl) => {
        ...
        switch (strngthLvl) {
            case 0:
                if (ptPassword.length > 6 && lowerUpperChk(ptPassword) && numChk(ptPassword)) {
                    return true;
                }
                else {
                    return false;
            ...
            case 1:
                ...
                if (ptPassword.length > 8 && lowerUpperChk(ptPassword) && symbolChk(ptPassword) && numChk(ptPassword)) {
                    return true;
                }
                else {
                    return false;
    ...
    const chkAdmin = async (userName) => {
        return ((await getUserType(userName)).toLowerCase() == 'admin')
    }
    ...
    const modUserByAdmin = async (userName, newPassword, newType, newRole) => {
        ...
        const modResult = await db.query(modUserQuery, [newPassHash, newType, newRole, userName])
